...
vector<int> getCode(string& text, const int& n) {
	...
	for (int i = 0; i < fN; ++i) {
		...
		int count{};
		while (in >> russianWord) {
			for (int ii = 0; ii < vstrings.size(); ++ii)
				if (vstrings[ii] == russianWord || vstrings[ii] == russianWord + ",") {
					++count;
					if (count > maxCount) {
						maxCount = count;
	...
	return codes.size() != 0 ? codes[codes.size()-1] : vector<int>();
}
...
void task2() {
	...
	std::cout << "Декодирование блочного шифра, введите размер ключа: ";
	int n; // размер кодового блока
	std::cin >> n;
	std::cout << "Процесс декодирования...\n";
	...
	if (code.empty())
		std::cout << "Текст не получилось расскодировать.\n"
					"Проверьте закодированный текст или попробуйте добавить русских слов в 10000-russian-words.txt\n";
	else {
		string res{};
